...
| [STKonline](https://www.stkonline.sk/) | EČV/VIN, značka, model, farba, palivo, vykonanie a platnosť TK/EK | Presná identita; termín kontroly sa nepreberie bez dátumu vykonania. Aktualizácia môže meškať; platené polia sa nečítajú. |
| [HAKA](https://www.hakasystem.eu/) | Presne zodpovedajúce verejné hlásenie a odkaz | Nenájdené hlásenie neznamená, že vozidlo nie je kradnuté. Kontakty ľudí sa neukladajú. |
...
- Voliteľný serverový `VEHICLE_LOOKUP_SIGNING_KEY` je stabilné tajomstvo na podpisovanie. Bez neho sa používa existujúci Supabase service-role kľúč. Zmena podpisového kľúča zneplatní staršie podpisy: overenia sa potom nezobrazia a treba ich dohľadať znovu. Pri rotácii service-role kľúča preto treba výslovne riešiť zachovanie podpisového kľúča; nastavenie nového odlišného kľúča nezachová staré podpisy.
...
- Najviac jedna externá operácia naraz na organizáciu, 5/min na profil a 30/min na organizáciu. Limit a rezervácia sa koordinujú transakčne v databáze naprieč Vercel procesmi. Súbežný lookup vráti 409 s `Retry-After`, limit 429. API vyžaduje aktívnu dispečerskú alebo vyššiu rolu.
