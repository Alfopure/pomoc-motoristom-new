...
Bridging asymmetry: inbound and outbound calls bridge **from the customer leg** with `park_after_unbridge: "self"`, so an operator drop parks the customer in the waiting room. Outbound calls, including callbacks, no longer play `outboundIntro`: without recording they bridge directly on customer `call.answered`, without waiting for any playback completion. When outbound recording is enabled, only its purpose-specific recording notice precedes capture and connection; recorded calls with verified conference support connect through the customer-anchored conference. Internal calls retain their existing bridge flow and ringback.
