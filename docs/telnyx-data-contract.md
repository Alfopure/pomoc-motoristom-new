...
Every Telnyx command carries `command_id = uuidv5("<sessionId>|<legId>|<step>|<intent>")` under a fixed namespace (`src/server/telephony/telnyx/command-id.ts`). Intents in use: `answer`, `bridge`, `bridge:own`, `bridge:rejoin`, `dial:outbound`, `dial:internal`, `dial:consult`, `dial:pickup`, `ring`, `hangup`, `playback:greeting`, `playback:moh`, `playback:all_busy`, `playback:callback_confirmed`, `playback_stop`, `gather_stop`, `transfer`, `conference:create`, `conference:join*`, `conference:leave*`, `conference:hold*`, `conference:unhold*`, `conference:mute`, `conference:unmute`, `conference:update:<mode>`, `dial:party`, `dial:supervise`. A retried webhook therefore replays the same command id and Telnyx deduplicates it. Telnyx accepts `command_id` only on the `join`, `leave`, `speak`, `update` and `end` conference actions; `hold`, `unhold`, `mute` and `unmute` are sent without it.
...
| `conference` | add-party (Phase 4) | promote; dial the third party with `link_to` and `client_state.intent = "party"`; the session becomes `conference` only when that leg answers and `join`s (a caller who was `held` while it rang is unheld then). Up to `MAX_CONFERENCE_PARTIES = 3` added parties. `mute`/`unmute` take `call_control_ids`; removing a party is `leave` + `hangup` on its leg and the ordinary `call.hangup` path brings the session back to `talking`/`held`. When the operator leaves (action or lost leg) the first remaining party takes the call over (`answered_by_profile_id`, `metadata.previous_operator`) |
| `waiting` / `parked` | IVR wait, overflow, operator park, lost operator leg | customer leaves the conference (or is unbridged); the music is one detached `playback_start {loop: "infinity"}` and the state machine's heartbeat is a separate silent `gather {timeout_millis: MOH_TICK_TIMEOUT_MS = 60 s}`, so re-arming the tick never interrupts the audio (the sweeper re-arms after `WAITING_TICK_STALE_MS` = 2 ticks, min 90 s); the row appears in the čakáreň together with `metadata.park.by` (who odložil the caller) and the countdown to the frozen `metadata.waiting.max_minutes`; pickup dials the picker's WebRTC leg and bridges; after `park_max_minutes` the callback prompt replaces the indefinite park, and the resulting `motorist_callback_requests` row (`source: 'park_timeout'`) lands in the callback queue |
...
Bridging asymmetry: inbound and outbound calls bridge **from the customer leg** with `park_after_unbridge: "self"`, so an operator drop parks the customer in the waiting room. Outbound calls, including callbacks, default to connecting directly on customer `call.answered`, without waiting for introductory playback or recording startup. Internal calls retain their existing bridge flow and ringback.

Startup announcements are configured independently for each line and direction in `metadata.announcements`: optional `inboundStartAnnouncements` defaults to `true`, and optional `outboundStartAnnouncements` defaults to `false`, including legacy saved configurations. The switches control both the greeting/service introduction and the initial recording notice. Turning startup announcements off also disables automatic recording for that call, without changing the organization's recording policy. Turning outgoing startup announcements on restores `outboundIntro` followed by the purpose-specific notice when recording is enabled. Settings are frozen for a new call; changing a line does not change calls already in progress. These flags use existing line metadata and need no migration. IVR menus, waiting music and the independent `recordingStatusAnnouncements` preference retain their own behavior.
...
Free-form JSON companion to the typed columns (`SessionMeta` in `src/server/telephony/state/types.ts`). Keys in use: `match` (top `CallerMatch`, count, degraded flag), `ring` (frozen `plan`, `mode`, `active_step`, `step_started_at`, `step_deadline_at`, `exhausted`, `fallback`), `outbound`, `internal`, `transfer`, `consult`, `callback`, `hangup`, `conference`, `park`, `ivr`, `after_hours`, `pickup`, `waiting` (`since`, `reason`, `ticks`, `last_tick_at`), `party_pending`, `supervise`, `line_label`, `partner_name`.

New in Phase 4 stage 3: `party_pending` (`{target, by, at}` while an added participant rings) and `supervise` (`{[supervisorProfileId]: {mode, at, by}}`). Supervision leaves the session state, `answered_by_profile_id` and the operator's presence untouched — a supervisor is an extra conference participant, never a party to the call.

`motorist_call_legs.metadata` carries `muted` (conference mute flag, written by the mute/unmute action) and `supervisor_mode` (`monitor` / `whisper` / `barge`, written when a supervisor leg joins); both are surfaced on `GET /api/telephony/calls/active` next to `client_state.intent`.

### Supervision (design §4 Phase 4)

`POST /api/telephony/calls/[id]/supervise {mode}` (manager/admin only, `canSuperviseRole`) promotes the call to its conference and dials the supervisor's own WebRTC leg (`role: "supervisor"`, `client_state.intent = "supervise"`, `autoAnswer`). On `call.answered` that leg joins with the verified Telnyx parameters `supervisor_role` (`barge | monitor | none | whisper`) and, for `whisper`, `whisper_call_control_ids: [operatorLeg]` so the caller never hears the supervisor. Calling the route again with another mode switches the role live through `POST /conferences/{id}/actions/update` instead of re-dialling. `stop-supervise` leaves the conference and hangs the supervisor leg up. The supervisor's presence is reserved **best effort**: an `available` manager is taken out of the ring plan while they listen, a paused or logged-out one may still supervise. When supervision (or the whole call) ends they go straight back to `available` — a supervisor only listened, so they are never given after-call work. Every start, switch and stop writes a `motorist_audit_log` row (`entity_type: "telephony_call"`, `entity_id` = session id, action `telephony.supervise.{start,switch,stop}`); the participant actions write `telephony.conference.{add_party,mute_party,unmute_party,remove_party,leave}`.

`callback_request_id` is added to the metadata of the **outbound** session started from the queue, and the request itself carries the mirror image under `metadata.callback_call` (`session_id`, `at`, `by`). Both writes are best effort: the call is already live when they run, so a failed bookkeeping write is reported as `linked: false` and logged rather than failing the callback.
...
| `GET /api/telephony/cron` | `bearer CRON_SECRET` | ledger replay first (hangup → answered → bridged → initiated → hold/unhold → gather/playback/speak.ended, ≤ 60 s from the cron start, deferred rows younger than 60 s included), then the ring sweep (≤ 25 sessions / 20 s, plus orphan-leg and leaked ring-offer cleanup), stuck-session detection, ledger prune; `maxDuration = 120`, every job reports `startedAt`/`ms` |
