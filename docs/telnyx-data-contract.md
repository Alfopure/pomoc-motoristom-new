...
Every operator leg is dialled from the line's own DID — the only verified origination number (runbook S3) — so the SIP `From` a browser phone sees is the partner line, never the person calling. Legs dialled or transferred towards a `sip:` destination therefore carry an `X-PM-Caller` invite header with the customer's E.164 number (`caller_number` of an inbound session, `called_number` of an outbound one; nothing on internal calls or PSTN legs). `invitePresentedCallerNumber` in `webphone-model.ts` prefers that header for the ringing summary and the OS notification and falls back to `From` when it does not survive the provider; the console's own panels keep reading the session's `caller_number`, so a missing header degrades to the old behaviour, never to a wrong number.
