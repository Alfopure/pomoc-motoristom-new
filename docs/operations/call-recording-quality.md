...
Prevádzkovateľ podľa pokynu vlastníka je **ALFOPURE s.r.o., IČO 11698055**, kontakt **info@alfopure.tech**. Verejná stránka `/ochrana-hovorov` obsahuje informácie a aktuálne lehoty bez prihlásenia. Kratšie slovenské oznámenie o nahrávaní a kvalite v4 používa všeobecné označenie spoločnosti a prirodzený hlas Richard; dostupné sú aj samostatné oznamy bez hodnotenia kvality a české, anglické a nemecké verzie.
...
### Verified customer conference connection

When conference recording is verified, the initial recorded connection creates a conference from the unbridged customer leg and joins the operator after the notice and recorder warmup. Repeated physical Telnyx probes found intermittent missing audio with bridge-to-conference promotion; the direct customer conference carried both directions, three-party audio, barge and attended transfer in two independent runs. A separate real incoming-leg probe also captured both channels. Private consultation and whisper remain outside recording segments.

The original durable connection command retains its own unique conference name and idempotent create/join IDs. Persist the returned conference ID under the organization/session/version guard before waiting for real joined membership. Membership polling is bounded, and each delayed action rechecks the lease, session epoch, objection, hangup and both legs. Lost acknowledgements keep the pending connection recoverable; they never run a compensation that claims the former bridge survived. Park and blind transfer retain the old conference reference just long enough to leave it.

Measured WAV duration still must match provider timing within 100 ms. Two short probe segments exceeded that tolerance and correctly remain ineligible for personal scoring; conference audio with three mixed speakers is not assigned to a named operator by voice guessing.
