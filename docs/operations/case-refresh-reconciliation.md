...
The standalone release was validated against dev after the P0 merge: 3,464 Vitest tests, 60 Node tests with local PostgreSQL, 18 mutation database tests and 33 isolated Chrome case scenarios passed. TypeScript, changed-file ESLint, the target-project guard and the Next 16 production build passed. External fleet smoke was excluded. Optional PDF inspection requires unavailable `pdfinfo`/`pdftotext`; bundled Chromium generated a valid PDF, but that inspection step could not complete. These checks made no shared-project writes or live calls.
