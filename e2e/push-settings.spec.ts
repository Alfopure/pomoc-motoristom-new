...
async function openPushSettings(page: Page, options: { ios?: boolean; standalone?: boolean; grant?: "granted" | "denied"; configured?: boolean; rejectSubscription?: boolean } = {}) {
  ...
  await page.route("**/api/**", async (route) => {
    ...
    if (method === "GET") {
      await route.fulfill({ status: 200, json: { configured: options.configured ?? true, publicKey: "BAEC", subscribed, soundEnabled } });
      return;
    }
    ...
    } else if (method === "PATCH") soundEnabled = body.soundEnabled as boolean;
  ...
  await page.route("**/push-settings-fixture", (route) => route.fulfill({
    ...
    body: `<!doctype html><html lang="sk"><head><meta name="viewport" content="width=device-width, initial-scale=1"></head><body><div id="root"></div><script>${fixtureScript}</script></body></html>`,
  }));
