...
function Fixture() {
  ...
  const [template, setTemplate] = useState<"custom" | "location_request">("custom");
  const global = new URLSearchParams(location.search).has("global");
  Object.assign(window, { changeSmsCase: () => setCaseId("case-2"), changeSmsTemplate: () => setTemplate("location_request") });
  return <main className="p-6">
    {global ? <DashboardPhone caseContext={{ id: caseId, caseNumber: "PM-123", phone: "+421905999999" }} onDial={() => undefined} onCreateCase={() => { document.body.dataset.created = "true"; }} />
      : <><button type="button" onClick={() => setOpen(true)}>Otvoriť SMS</button><SmsComposerDialog initialTemplate={template} caseId={caseId} caseNumber={caseId === "case-1" ? "PM-123" : "PM-456"} initialPhone="+421905999999" open={open} onClose={() => setOpen(false)} /><CaseSmsHistory caseId="case-1" /><UseCustomerLocationButton caseId="case-1" expectedUpdatedAt="2026-09-07T11:00:00.000Z" location={{ lat: 48.1, lng: 17.1, label: "GPS", submittedAt: "2026-09-07T12:00:00Z" }} onNotice={() => undefined} onApplied={() => undefined} /></>}
  </main>;
}
