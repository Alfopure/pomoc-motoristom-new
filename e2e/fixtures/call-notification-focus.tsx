...
function Fixture() {
  ...
  const currentCall = scenario === "internal" ? { ...call, ...pending, kind: "offer" as const, state: "ringing" as const, direction: "internal" as const, offeredToMe: false }
    ...
    : recovery ? { ...call, kind: "offer" as const, state: "ringing" as const, offeredToMe: reservedOffer }
  ...
  return <main className="min-h-dvh bg-zinc-50 p-3">
    <CallNotificationFocus
      focus={focus}
      model={{ ...buildPhoneBarModel(EMPTY_ACTIVE_CALLS), checkedAt: "fresh-snapshot", configured: true, ownPresenceStatus: reservedOffer ? "ringing" : "available", teamCalls: scenario === "ended" ? [] : [currentCall], presence: {
        actorProfileId: "me", checkedAt: "fresh-snapshot", canManageAssignments: false, devices: [],
        presence: [{ profileId: "me", status: reservedOffer ? "ringing" : "available", currentSessionId: scenario === "other-offer-recovery" ? "other-session" : reservedOffer ? sessionId : null }],
      } }}
      phone={{ ...phone, pendingOperatorLegs: scenario === "pending-recovery" ? 1 : 0, call: incoming ? {
