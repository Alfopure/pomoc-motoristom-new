...
function TaskWorkspaceSession({ children, actorKey, enabled = false, initialTasks, viewerProfileId, onEditorStateChange, onTasksChange }: TaskWorkspaceProviderProps) {
  ...
  useEffect(() => { if (snapshot.hidden) onTasksChange?.([]); }, [snapshot.hidden, onTasksChange]);
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      ...
      inFlight = true;
      try { await store.refresh(); const id = store.getSnapshot().selectedId; if (id) await store.loadMessages(id); }
    ...
    void refresh();
    const resume = () => { store.checkAuthorization(); if (document.visibilityState === "visible") void store.reauthorize(); };
    ...
    try {
      ...
      const { data } = client.auth.onAuthStateChange((event, session) => {
        if (!session || event === "SIGNED_OUT" || (userId && session.user.id !== userId)) { store.clear(); cleanupChannel?.(); }
        if (session?.user.id) userId = session.user.id;
      });
      cleanupAuth = () => data.subscription.unsubscribe();
      const organizationId = actorKey?.split(":")[0];
      if (organizationId) {
        const channel = client.channel(`tasks:${organizationId}`, { config: { private: true } }).on("broadcast", { event: "invalidate" }, schedule).subscribe(status => { if (status === "SUBSCRIBED") schedule(); });
    ...
    return () => { stopped = true; window.clearTimeout(broadcastTimer); window.clearInterval(interval); window.removeEventListener("focus", resume); window.removeEventListener("online", resume); document.removeEventListener("visibilitychange", resume); cleanupAuth?.(); cleanupChannel?.(); store.clear(); };
