...
import { CalendarDays, CheckCircle2, Clock3, GripVertical, RotateCcw } from "lucide-react";
...
export function TaskWorkspaceCard({ task, operators, selected, dirty, disabled, saving, draggable = false, now, onSelect, onStatusChange }: CardProps) {
  ...
  return <li ref={setNodeRef} data-task-id={task.id} aria-busy={saving || undefined}
    ...
    <button type="button" data-task-status-action className={styles.statusAction} disabled={disabled} aria-label={`${action} úlohu ${task.title}`}
