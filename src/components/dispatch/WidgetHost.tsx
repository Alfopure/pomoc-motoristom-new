  ...
  return <aside id="dispatch-tools-panel" aria-label="Nástroje" data-settings-open={settingsOpen} onKeyDown={event => { if (event.key === "Escape") { event.preventDefault(); event.stopPropagation(); onClose(); } }} className={`dispatch-widget-host min-h-0 min-w-0 flex-col border-l border-zinc-200 bg-white ${expanded ? "is-open" : ""}`}>
    ...
    <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain">
      {settingsOpen && <section id="widget-settings" aria-label="Nastavenie widgetov" className="widget-settings space-y-1 border-b border-zinc-200 bg-zinc-50 p-3">
        ...
        {widgets.map((widget, index) => <div key={widget.id} draggable onDragStart={() => setDragged(widget.id)} onDragEnd={() => setDragged(null)} onDragOver={event => event.preventDefault()} onDrop={event => { event.preventDefault(); if (dragged) move(dragged, index); setDragged(null); }} className="flex min-w-0 items-center gap-1 rounded-md border border-zinc-200/70 bg-white px-1">
      ...
      {widgets.map(widget => <section key={widget.id} hidden={!widget.visible} data-widget={widget.id} className="border-b border-zinc-200">
