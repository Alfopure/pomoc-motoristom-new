            ...
            <div className="grid gap-3 rounded-md border border-zinc-200 p-3 sm:p-4">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <label className="flex min-h-11 cursor-pointer items-center justify-between gap-4 rounded-md border border-zinc-200 bg-zinc-50 p-3">
                  <span className="min-w-0"><span className="block text-sm font-semibold text-zinc-900">Prichádzajúce hovory</span><span id={`inbound-start-hint-${line.id}`} className="mt-1 block text-xs leading-relaxed text-zinc-500">{config.inboundStartAnnouncements === true ? "Zapnuté — volajúci najprv počuje úvodné hlášky." : "Vypnuté — hovor pokračuje bez úvodných hlášok."}</span></span>
                  <input type="checkbox" role="switch" aria-label="Úvodné hlášky prichádzajúcich hovorov" aria-describedby={`inbound-start-hint-${line.id}`} checked={config.inboundStartAnnouncements === true} disabled={!canEdit || saving} onChange={(event) => updateConfig({ ...config, inboundStartAnnouncements: event.target.checked })} className="h-5 w-5 shrink-0 cursor-pointer accent-zinc-900 disabled:cursor-not-allowed" />
                </label>
                <label className="flex min-h-11 cursor-pointer items-center justify-between gap-4 rounded-md border border-zinc-200 bg-zinc-50 p-3">
