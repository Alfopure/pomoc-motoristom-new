...
import { HandoffReadError, readHandoff, useHandoffRead } from "./use-handoff-read";
...
export function HandoffRecipient() {
  ...
  const reader = useHandoffRead({
    ...
    read: async signal => {
      ...
      // The session exchange is attempted once; background retries can only use GET.
      token.current = null;
      if (bearer) {
        try {
          return await readHandoff<{ handoff: CaseHandoff }>("/api/public/handoffs/session", signal, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ token: bearer }) });
        } catch (caught) {
          if (caught instanceof HandoffReadError && [401, 403, 404, 410].includes(caught.status)) throw caught;
          throw new HandoffReadError("Overenie odkazu sa nepodarilo dokončiť. Otvorte znovu pôvodný odkaz od dispečera.", 410);
        }
      }
      if (!expectedGrant.current) throw new HandoffReadError("Otvorte celý odkaz, ktorý vám poslal dispečer.", 410);
      return readHandoff<{ handoff: CaseHandoff }>(`/api/public/handoffs/current?handoff=${encodeURIComponent(expectedGrant.current)}`, signal);
    },
    accept: data => {
    ...
    onDenied: () => { setHandoff(null); },
  });
  const stopRead = reader.stop;
  useEffect(() => {
    if (!handoff) return;
    const checkExpiry = () => {
      if (Date.parse(handoff.expiresAt) <= Date.now()) stopRead("Platnosť odkazu skončila. Kontaktujte dispečera.");
    };
    checkExpiry();
  ...
  async function send(action: string) {
    if (!handoff || inFlight.current || reader.halted) return;
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if ([401, 403, 404, 410].includes(response.status)) reader.stop();
  ...
  return <main className="handoff-page"><header className="handoff-public-brand"><span>PM</span><div><strong>Pomoc motoristom</strong><p>Prípad pre externého kolegu</p></div></header>
    <article className="handoff-public-card">
      {!handoff ? <><h1>{reader.loading ? "Načítavam kartu…" : "Odovzdanie prípadu"}</h1><p>Prístup je určený iba príjemcovi konkrétneho odkazu.</p></> : <>
      ...
      {reader.error && <p role="alert" className="handoff-error">{reader.error}{!reader.halted && <button disabled={reader.loading || busy} onClick={reader.retry}>Skúsiť znova načítať</button>}</p>}
      {pending && !reader.halted && <button disabled={busy} onClick={() => void send(pending.action)}>{busy ? "Overujem výsledok…" : "Overiť výsledok tej istej požiadavky"}</button>}
