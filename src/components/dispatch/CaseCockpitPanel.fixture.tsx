...
function Fixture() {
  ...
  if (view === "drawer") return <><button onClick={() => setOpen(true)}>Otvoriť drawer</button><CaseDrawer caseItem={currentCase} assets={[]} branches={[]} partnerDirectory={[]} priceRule={priceRule} open={open} onClose={() => setOpen(false)} onCaseChange={detail => setCurrentCase(current => ({ ...current, ...detail, tasks: current.tasks }))} /></>;
  if (view === "detail") return <div style={{ height: "100dvh" }}><ExpandedCasePanel assets={[]} branches={[]} call={call} caseItem={currentCase} commanderVehicles={[]} kind="detail" operators={[]} partnerDirectory={[]} onBackToCockpit={noop} onCaseCreated={noop} onDataChange={onDataChange} onCaseChange={detail => setCurrentCase(current => ({ ...current, ...detail, tasks: current.tasks }))} /></div>;
  return <div style={{ height: "100dvh", display: "flex", flexDirection: "column" }}><CaseCockpitPanel assets={[]} branches={[]} caseItem={currentCase} commanderVehicles={[]} focusedTaskId={new URLSearchParams(location.search).get("task") ?? undefined} mode={mode} model={createDispatchMapModel(currentCase, [], [])} operators={[]} partnerDirectory={[]} onCollapse={() => setMode("collapsed")} onExpand={() => setMode("expanded")} onRestore={() => setMode("split")} onDataChange={onDataChange} onCaseChange={detail => setCurrentCase(current => ({ ...current, ...detail, tasks: current.tasks }))} onDirtyChange={noop} onSaveDraftChange={noop} onSavingChange={noop} /></div>;
}
