...
import { enqueueWaitingCallPickupCommand } from "@/server/telephony/call-commands";
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const body = (await request.json().catch(() => null)) as PickupBody | null;
    const { id } = await context.params;
    const leaseFence = body?.leaseId === undefined ? undefined : readWorkplaceLeaseFence(body);
    const command = await enqueueWaitingCallPickupCommand(actor, id, body?.targetExtension, leaseFence);
