 ...
 * with 409 until the browser repeats it with `{ takeover: true }`.
 */
export async function POST(request: Request) {
  try {
    ...
    const body = await readJsonBody<{ takeover?: unknown }>(request).catch(() => ({}) as { takeover?: unknown });
    ...
    const token = await issueWebphoneToken(
      ...
      { organizationId: deps.organizationId, profileId: actor.profileId, userAgent: request.headers.get("user-agent"), takeover: body?.takeover === true },
    );
