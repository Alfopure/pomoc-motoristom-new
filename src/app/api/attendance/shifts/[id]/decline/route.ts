...
import { declineAttendanceShift, MutationError } from "@/server/motorist-mutations";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const shift = await declineAttendanceShift(id, input.note);
...
function mutationErrorResponse(error: unknown) {
  ...
  console.error("Attendance decline failed:", error);
  return Response.json({ error: "Službu sa nepodarilo odmietnuť." }, { status: 500 });
}
