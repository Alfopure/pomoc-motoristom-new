...
describe("ivr through the pipeline", () => {
  ...
  it("replays the menu after an invalid digit and gives up on the second one", async () => {
    ...
    expect(replay.params).toMatchObject({ audioUrl: "https://media.test/telephony/announcements-v4/sk/ivr-main.mp3", invalidAudioUrl: "https://media.test/telephony/announcements-v4/sk/invalid-input.mp3" });
  ...
  it("plays the closing message of a hangup option and ends the call after it", async () => {
    ...
    expect(h.telnyx.of("playbackStart").at(-1)?.params.audioUrl).toBe("https://media.test/telephony/announcements-v4/sk/after-hours.mp3");
