...
async function findSession(admin: AdminClient, organizationId: string, event: TelephonyEvent): Promise<SessionRow | null> {
  ...
  if (event.callControlId) {
    const leg = await admin.from("motorist_call_legs").select("session_id").eq("telnyx_call_control_id", event.callControlId).maybeSingle();
    if (leg.error) throw new Error(`leg lookup failed: ${leg.error.message}`);
    if (leg.data) {
      const byLeg = await admin.from("motorist_call_sessions").select("*").eq("id", leg.data.session_id).maybeSingle();
...
async function findLine(admin: AdminClient, organizationId: string, environment: string, to: string | null): Promise<LineRow | null> {
  const normalized = to ? normalizeE164(to) : null;
  if (!normalized) return null;
  const { data, error } = await admin.from("motorist_telephony_lines").select("*").eq("organization_id", organizationId).eq("phone_number", normalized).eq("active", true);
  if (error) throw new Error(`line lookup failed: ${error.message}`);
  const rows = data ?? [];
  return rows.find((row) => row.environment === environment) ?? rows[0] ?? null;
}
...
export async function createInboundSession(deps: ProcessorDeps, event: TelephonyEvent): Promise<SessionRow> {
  ...
  const line = await findLine(admin, organizationId, deps.environment, event.to);
...
export async function processTelnyxEvent(deps: ProcessorDeps, envelope: unknown): Promise<ProcessorResult> {
  ...
  try {
    ...
    try {
      await markWebhookEventFailed(deps.admin, event.id, error);
    } catch (ledgerError) {
    ...
    return done({ ...identity, claim, sessionId: session?.id ?? null, status: eventClass === "control" ? 200 : 500, outcome: "failed", error: message });
  }
}
