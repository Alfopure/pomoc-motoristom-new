...
describe('bounded durable coordinator',()=>{
 it('keeps 15s processing cap, 110s total cutoff and refuses work after live jobs consume reserve',async()=>{
  expect(recordingProcessingDeadline(0,10000)).toBe(25000);expect(recordingProcessingDeadline(0,45000)).toBe(60000);
  // Tail slot of the 120 s cron: started ~95 s in (after replay 60 s + sweep 20 s) it still gets its budget, capped at 110 s.
  expect(recordingProcessingDeadline(0,95000)).toBe(110000);expect(recordingProcessingDeadline(0,100000)).toBe(110000);
  const from=vi.fn();const result=await runRecordingProcessing({admin:{from} as unknown as RecordingAdmin,organizationId:'org',cronStartedAt:Date.now()-108000});
