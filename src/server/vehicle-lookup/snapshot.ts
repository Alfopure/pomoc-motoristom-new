...
export function verifyVehicleLookup(value: unknown, organizationId: string, identity: VehicleIdentity, key?: string): VehicleLookupSnapshot | null {
  ...
  // Explicit keys (e.g. isolated tests) never fall back to configured secrets.
  const keys = key !== undefined ? [key] : [signingKey(), process.env.VEHICLE_LOOKUP_PREVIOUS_SIGNING_KEY].filter((value): value is string => Boolean(value));
  const valid = keys.map(candidate => {
    const expected = Buffer.from(sign(snapshot.result, organizationId, candidate));
    return actual.length === expected.length && timingSafeEqual(actual, expected);
  }).some(Boolean);
  if (!valid) throw new Error("Výsledok dohľadania sa nepodarilo overiť. Dohľadajte vozidlo znova.");
