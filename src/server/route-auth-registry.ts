...
export const ROUTE_AUTH_REGISTRY: Record<string, RouteAuthEntry> = {
  ...
  "telephony/config/business-hours": { class: "session", role: ["manager", "admin"], note: "GET je member-level (CONFIG_READ_ROLES); PUT manager/admin cez handleConfigWrite." },
  "telephony/config/numbers": { class: "session", role: ["manager", "admin"], note: "GET je member-level; PATCH manager/admin. Kúpa čísla cez Telnyx nie je súčasťou tejto fázy." },
  "telephony/config/pause-reasons": { class: "session", role: ["manager", "admin"], note: "GET je member-level; PUT manager/admin." },
  "telephony/config/ring-groups": { class: "session", role: ["manager", "admin"], note: "GET je member-level; PUT manager/admin (transakčná výmena cez motorist_replace_ring_plan)." },
  "telephony/config/ring-plans": { class: "session", role: ["manager", "admin"], note: "GET je member-level; PUT manager/admin (transakčná výmena cez motorist_replace_ring_plan)." },
  "telephony/config/settings": { class: "session", role: ["admin"], note: "Kill switche, allowlist cieľov a limit čakárne — GET aj PATCH len admin." },
  "telephony/operators/[id]/credential": { class: "session", role: ["manager", "admin"], note: "Vytvorí alebo pregeneruje Telnyx SIP credential operátora." },
  "telephony/operators/[id]/disconnect": { class: "session", role: ["manager", "admin"], note: "Odpojí prehliadačový telefón operátora (ďalší heartbeat dostane 409)." },
