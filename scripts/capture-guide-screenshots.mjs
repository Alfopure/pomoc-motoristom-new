...
try {
 ...
 const settingsShots = [
  ...
  ['announcements', 'Hlášky a jazyk', 'Výber linky, jazyka a samostatné prepínače úvodných hlášok pre oba smery hovoru.', [['language', 'Jazyk hlášok', p => p.getByText('Jazyk hovoru', { exact: false })], ['inbound-start', 'Úvodné hlášky prichádzajúcich hovorov', p => p.getByRole('switch', { name: 'Úvodné hlášky prichádzajúcich hovorov', exact: true })], ['outbound-start', 'Úvod odchádzajúcich a spätných hovorov; vypnutie blokuje aj automatické nahrávanie', p => p.getByRole('switch', { name: 'Úvodné hlášky odchádzajúcich hovorov', exact: true })]]],
